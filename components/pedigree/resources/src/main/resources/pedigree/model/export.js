  ...
  PedigreeExport.exportAsPED = function(pedigree, idGenerationPreference, selectedDisorders)
  {
     ...
     for (var i = 0; i <= pedigree.GG.getMaxRealVertexId(); i++) {
         ...
         if (!selectedDisorders) {
             ...
             var intersection = selectedDisorders.filter(function (item) { return nodeDisorders.indexOf(item) > -1;});
             //if node is affected of a selected disorder
             if (intersection > 0 && pedigree.GG.properties[i]["carrierStatus"] == "affected")
                     status = 2;
                 else
                     status = 1;
         }
